...
try {
   ...
   $mail->setFrom('gameroom@gameatl.com', 'Southern-Fried Gaming Expo');
   $mail->addAddress($email, $fullname);
   $mail->Subject = 'SFGE: Game Registration Login Link';
   $mail->Body = $email_template;
   $mail->send();
}
catch (Exception $e)
...
<html lang="en-US">
  <head>
    ...
    <title>Document title</title>
			...
			<h2>Machine Down - LIST</h2>
            
    
			<table width=100%>
			<tr><th>ID</th><th>Machine Name</th></tr>
            <?php
            if ($downlist)
            foreach ($downlist as $downlist) {
              echo "<tr><td>". $downlist['machineid']."</td><td><a href='https://gameatl.com/machine-update/?id=" . $downlist['Id']."&mid=". $downlist['machineid']."&na=".urlencode($downlist['gametitle'])."'>" . $downlist['gametitle']. "</a></td></tr>";
            } else {
