...
$grid["edit_options"]["success_msg"] = "Machine updated";
$grid["delete_options"]["success_msg"] = "Machine deleted";
...
$g->table = "gamelist";
$g->select_command = "SELECT games.gamelistid, games.yearlistid, games.approved, games.tournamentpin, games.emailed, games.gametitle, games.gametype, user.firstname, user.lastname, games.ownerid, games.builtyear, games.manufacturer, games.awards, IF(LENGTH(games.notes) > 0, 'Yes', '') as HasNotes, games.notes 
...
$buttons_html = '<a class="ui-custom-icon ui-icon ui-icon-mail" title="Message User" onClick="myPopup({gamelistid}, {ownerid});")></a>';
...
//$f["op"] = "eq";
//$f["value"] = "0";
//$f["cellcss"] = "'background-color':'#ffb8a8','border':'1px solid darkgray','color':'white'"; 
//$f["target"] = "gametitle";
//$f_conditions[] = $f;
...
{
window.open("https://gameroom.gameatl.com/admin/","_self")
},
position: "first"
});
},10);
...
<script>
    function myPopup(gamelistid, ownerid) {
    var left = (screen.width - 1050) / 2;
    var top = (screen.height - 550) / 4;
    var myWindow = window.open("https://gameroom.gameatl.com/admin/message.php?user="+ownerid, 'Send Text', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=1050, height=550, top=' + top + ', left=' + left);
    }
</script>
        <style>
