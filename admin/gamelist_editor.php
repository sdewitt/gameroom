...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update'])) {
        $stmt = $pdo->prepare("UPDATE gamelist SET gametitle=?, gametype=?, builtyear=?, notes=?, ownerid=? WHERE gamelistid=? AND showyear='2025'");
        $stmt->execute([
        ...
        $ids2025 = $pdo->query("SELECT yearlistid FROM gamelist WHERE showyear = '2025' ORDER BY yearlistid")->fetchAll(PDO::FETCH_COLUMN);
        $nextID = 1;
        ...
        $stmt = $pdo->prepare("DELETE FROM gamelist WHERE gamelistid=? AND showyear='2025'");
